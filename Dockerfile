...
CMD php artisan config:clear && php artisan migrate --force && php artisan shield:generate --all --panel=admin -n && php artisan db:seed --class=AdminSeeder --force -n && php artisan config:cache && php artisan route:cache && php artisan view:cache && php artisan serve --host=0.0.0.0 --port=${PORT:-8080}
